...
describe("SpaceManager lifecycle", () => {
  ...
  it("creates, renames, archives, and preserves context for a space", async () => {
    const events = [];
    ...
    const space = await manager.createSpace(repoDir, {
      name: "Feature space",
  ...
  it("pins a space independently and emits the updated space", async () => {
    const space = await manager.createSpace(repoDir, { name: "Pinned space" });
    const updates = [];
  ...
  it("auto-commits dirty worktrees, squash-merges, and persists completion metadata", async () => {
    const space = await manager.createSpace(repoDir, { name: "Complete me" });
    writeFileSync(join(space.worktreePath, "feature.txt"), "from the space\n");

    const result = await manager.completeSpace(space.id, {
      mergeMethod: "squash",
  ...
  it("lists external worktrees as convertible and excludes already-spaced ones", async () => {
    ...
    const before = await manager.listConvertibleWorktrees(repoDir);
    assert.equal(before.length, 1);
    assert.equal(before[0].branch, "external-branch");

    // Native space worktree should not appear as convertible
    await manager.createSpace(repoDir, { name: "Native" });
    const afterCreate = await manager.listConvertibleWorktrees(repoDir);
    ...
    await manager.convertWorktreeToSpace(repoDir, before[0].path, {
      name: "Adopted",
    });
    assert.equal((await manager.listConvertibleWorktrees(repoDir)).length, 0);
  });

  it("converts an external worktree into a full-lifecycle space and refuses double-convert", async () => {
    ...
    const space = await manager.convertWorktreeToSpace(repoDir, externalWorktree, {
  ...
  it("refuses to convert the primary worktree", async () => {
    await assert.rejects(
